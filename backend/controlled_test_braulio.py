#!/usr/bin/env python3
"""
controlled_test_braulio.py — Controlled baseline extraction for braulio_resume.pdf.
...
async def run_test():
    file_path = "data/test_resumes/braulio_resume.pdf"
    if not os.path.exists(file_path):
        print(f"File not found: {file_path}")
        sys.exit(1)

    print("=" * 70)
    print("CONTROLLED GENERALIZATION TEST: braulio_resume.pdf")
    print("=" * 70)
    ...
    # 6. Hybrid employment recovery pass
    if llm_extraction:
        from app.parsers.employment_recoverer import recover_missing_employment
        llm_extraction.experiences = await recover_missing_employment(pdf_res.text, llm_extraction.experiences)

    # 7. Normalize skills
    if llm_extraction and llm_extraction.skills:
        llm_extraction.skills = normalize_skills(llm_extraction.skills)

    # 7. Override contacts and name
    if llm_extraction:
    ...
    print(f"Latest Company:          {latest_employer}")
    print(f"Latest Designation:      {latest_title}")
